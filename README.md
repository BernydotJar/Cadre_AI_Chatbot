...
Vercel deployment scope is authorized for Cadre_AI / cadre-ai3, but the currently connected Vercel integration exposes no project and its deploy action has a schema/runtime mismatch; the sandbox CLI is also logged out. [Deployment evidence](evidence/N6-release/public-drift-after-polish-20260909.md) demonstrates that the known alias is stale. GitHub CI/CD is now versioned: CI is secret-free, while production CD requires the existing Vercel org/project/token through the GitHub `production` environment and refuses to create a replacement project. The local branch is ahead of `origin/main`, but the audited publication action currently lacks its injected GitHub token, so these workflows are not active remotely yet. No paid add-on, source publication or final submission is implied.
