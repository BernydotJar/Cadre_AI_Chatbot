...
**The chatbot is now productized locally as a reusable Knowledge → Persona → Experience runtime.** Cadre composes the English persona **Donna**: she answers from the same verified factual authority and may add at most one application-owned diagnostic question after a grounded answer; users can explicitly ask for no follow-up. The generic shell derives identity, copy, theme, avatar and composer prompt from `ExperienceProfile`, and a fictional Acme/Scout fixture proves second-profile reuse without client-name branching. Current local verification is lint + strict typecheck + production build + **279/279 Vitest tests** + **52/52 Playwright desktop/mobile cases**. The existing [public chatbot](https://cadre-ai-chatbot-tawny.vercel.app) still represents the previously verified core release until this productized line is promoted and rechecked; see the [checkpoint](progress/checkpoint.md). The optional Chrome preview remains 72/72 + 23/23 + 17/17 PASS at its declared scope, while remote GitHub CI activation remains blocked by the audited publisher credential.
