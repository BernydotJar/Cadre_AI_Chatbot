...
The public production baseline remains PX6 revision 12 while the final invitation increment completes release. The current revision-15 runtime source `af7e3ff91ae91fc231defe87fde0139bd1c7f298` is locally closed by Graph Harness with independent source review, **302/302 Vitest, 70/70 Playwright, 73/73 extension tests, 24/24 synthetic extension checks**, production build, and existing-project Vercel prebuild PASS. Revision 15 keeps the <=430px launcher icon-only while making the separate Donna invitation compact, visible, and readable on mobile. Public equivalence is not inherited from r12; r15 still requires audited publication, CI, deployment to the same existing Vercel project, and fresh anonymous verification. Automated GitHub production CD remains separately blocked only because the GitHub `production` environment lacks the existing Vercel binding values.
