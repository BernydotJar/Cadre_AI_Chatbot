...
**The chatbot is now productized and publicly verified as a reusable Knowledge → Persona → Experience runtime.** Cadre composes the English persona **Donna**: she answers from the same verified factual authority and may add at most one application-owned diagnostic question after a grounded answer; users can explicitly ask for no follow-up. The generic shell derives identity, copy, theme, avatar and composer prompt from `ExperienceProfile`, and a fictional Acme/Scout fixture proves second-profile reuse without client-name branching. Current premium-source verification is lint + strict typecheck + production build + **286/286 Vitest tests** + **58/58 Playwright desktop/mobile cases**. The previously promoted productized line remains separately verified by a **52/52** anonymous matrix against the existing [public chatbot](https://cadre-ai-chatbot-tawny.vercel.app), whose Donna/product markers and bounded API behavior were reverified after exact-project promotion. The optional Chrome preview remains 72/72 extension tests + 24/24 repaired synthetic-browser checks + 17 scoped installed-site checks at its declared scope. Audited source publication and remote GitHub CI are now active; production CD intentionally remains blocked until the premium Graph state is releasable and the existing Vercel binding is supplied through GitHub `production` secrets.

The premium experience graph is now complete: PX2 Donna identity, PX3 ambient media, PX3B shell refinement, PX4 contextual Donna, and PX5 release integration are DONE with their required gates PASS. `npm run release:gate` passes locally. Production intentionally remains on the previously verified productized line until the final premium closure SHA is published, deployed to the existing Vercel project, and anonymously reverified.
