...
| M11 | Premium Donna experience | premium graph `PX2-donna-premium-identity` → `PX3-ambient-media` + `PX4-contextual-donna` → `PX5-premium-release` | **PX2 DONE / PX3 SECOND FIXER COMPLETE**: the first PX3 critic rejected imperceptible motion; the repaired asset/content then passed those concerns but a second critic found mobile Pause overlap and missing 760/320-reduced evidence. The layout now reserves control space, automated non-overlap checks cover 320/360/760, and final verification is 286/286 + 58/58. A fresh critic is required before PX3 design-review can pass; PX4 remains READY and PX5 stays locked. |
...
Execute PX3 ambient media first as the highest-priority READY premium node, then PX4 contextual Donna. Do not promote premium WIP to production before PX5 and the repository release gate pass.
