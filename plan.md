...
**Status: PX6_REVISION_15_DONE_LOCAL + PUBLIC_EQUIVALENCE_PENDING + GITHUB_CI_ACTIVE + AUTOMATED_CD_BINDING_BLOCKED.** Revision 12 remains the current public baseline. The owner-authorized r13 invitation increment was repaired through retained r13 68/70 mobile failure -> r14 independent critic CHANGES_REQUESTED -> r15 readable mobile repair. Exact runtime source `af7e3ff91ae91fc231defe87fde0139bd1c7f298` now has all local premium gates PASS and a clean detached verifier: 302/302 Vitest, 70/70 Playwright, 73/73 extension tests, 24/24 synthetic extension browser, build/typecheck/lint/Graph PASS. Existing-project Vercel production prebuild also passes with zero tracked drift. r15 must still be published, pass CI, deploy to the same Vercel project, and earn fresh public equivalence before replacing r12 as the public release. Final source-package verification follows that release. Automated GitHub CD remains separately blocked because the `production` environment lacks the existing Vercel org/project/token binding.
...
| M12 | Cadre-native cinematic + proactive Donna | premium graph `PX6-cinematic-proactive-donna` | **revision 15 DONE locally; public r15 equivalence pending**: r13 invitation work exposed two mobile failures (68/70), r14's first visual repair was independently rejected for sub-12px important copy, and r15 preserves the icon-only mobile launcher while making the separate nudge compact/readable. Exact source `af7e3ff` passes 302/302 Vitest, 70/70 Playwright, 73/73 extension tests, 24/24 synthetic browser, build/typecheck/lint/Graph, all three premium gates, repository release gate, and existing-project Vercel prebuild. Publication/CI/deployment/public equivalence still required. |
