...
| M11 | Premium Donna experience | premium graph `PX2-donna-premium-identity` → `PX3-ambient-media` → `PX3B-premium-shell-refinement` + `PX4-contextual-donna` → `PX5-premium-release` | **PX2 DONE / PX3 DONE / PX3B RUNNING**: PX3 survived two independent repair loops (imperceptible motion, then mobile Pause overlap/coverage), and now holds design-review/verification/integration-proof PASS with 286/286 unit tests and 58/58 Playwright independently rerun on the committed snapshot. Owner visual review then authorized a new forward-only PX3B refinement to remove remaining synthetic AI-badge language and raise the chat shell/material hierarchy before PX4/PX5. PX4 remains READY; PX5 now depends on PX3B as well as PX3/PX4. |
...
Execute PX3B premium shell refinement before PX4 because the owner explicitly raised visual-quality findings after PX3 closure. Then execute PX4 contextual Donna and PX5 integration/release. Do not promote premium WIP to production before PX5 and the repository release gate pass.
