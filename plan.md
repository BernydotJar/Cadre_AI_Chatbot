...
| M12 | Cadre-native cinematic + proactive Donna | premium graph `PX6-cinematic-proactive-donna` | **revision 12 extension-link integration repair in progress; r10/r11 evidence retained historically**: website-first Cadre shell, floating signal-orb Donna, verified `publicHighlights`, quick prompts, shaping state, empathetic deterministic handoffs, trust explainer, simplified favicon, and release-marker updates previously reached revision 10 local gate PASS (301/301 Vitest, 68/68 Playwright) and CI PASS on `69c590b` (run `34442075458`), with deploy-production run `34442247941` stopped only by the missing Vercel production binding. Revision 11 repairs stale `portal`/`maturity-index`/`track-ai-results` facts and links and adds a generic delegated-domain allowlist; full critic/fixer/verifier/Graph-gate closure for this revision remain pending. |
...
Claude Code 2.1.266 produced the first bounded Fixer draft without running tests. Coordinator review caught a representation bug in that draft: it compared `url.hostname` values against unparsed full domain URLs, which would still have rejected valid links. The accepted source normalizes configured domain URLs to hostnames before matching. No revision-12 tests/build/browser run is claimed until the fresh source critic completes and the final verifier is rerun.
