...
**Status: PX6_REVISION_12_PUBLIC_PASS + FINAL_SOURCE_PACKAGE_PENDING + GITHUB_CI_ACTIVE + AUTOMATED_CD_BINDING_BLOCKED.** The released baseline remains premium PX5 revision 4 on the existing Vercel project with anonymous 58/58 Playwright PASS. PX6 previously reached **revision 10 DONE locally** (all three premium gates PASS; clean verifier 301/301 Vitest + 68/68 Playwright; audited source `69c590b` passed GitHub CI run `34442075458`); deploy-production run `34442247941` then failed closed at the same missing Vercel org/project/token binding as G10 — no PX6 public deployment or public equivalence has ever been claimed. On 2026-09-10 the Graph reopened PX6 to `repair_required` at **revision 11**: a bounded source-freshness/documentation-consistency repair correcting stale `portal`/`maturity-index`/`track-ai-results` facts and links against the live public Cadre site (its "Get Your AI Results" CTA now resolves to a delegated `portal.gocadre.ai` domain, not the contact page), and adding a generic, explicit `additionalOfficialDomains` allowlist so that delegated domain can be approved without weakening rejection of undeclared foreign domains. Revision-10 local gates/CI evidence is retained as historical. Revision 11 passed its independent source critic and then reached the final verifier, where the app itself passed 302/302 Vitest, production build, and 68/68 Playwright before the optional extension build exposed a stale hardcoded `https://cadre.ai` link-origin assumption. The Graph retained that failure and opened revision 12 for a minimal adapter-build repair. Revision 12 is **DONE + PUBLIC PASS** with fresh `design-review=PASS`, `verification=PASS`, and post-release `integration-proof=PASS`. Exact source `8043886` passed GitHub CI `34446809913`, deployed through the existing Vercel project as `dpl_FGqGheG1GSx8EmUEJvd4rhEsdKYh`, passed anonymous markers, and passed the complete external **68/68** matrix when the unchanged 10-requests/60s per-client production rate window is respected (34/34 desktop, fresh window, 34/34 mobile). Two monolithic 67/68 attempts remain retained as correct 429/retry evidence rather than being erased. Final source-package verification remains. Automated GitHub CD remains independently blocked because the `production` environment lacks the existing Vercel org/project/token binding.
...
| M12 | Cadre-native cinematic + proactive Donna | premium graph `PX6-cinematic-proactive-donna` | **revision 12 DONE + public equivalence PASS; final source package pending**: website-first Cadre shell, floating signal-orb Donna, verified `publicHighlights`, quick prompts, shaping state, empathetic deterministic handoffs, trust explainer, simplified favicon, current Track-results/Maturity destination, and extension-link parity. Fresh r12 gates are all PASS; detached verification: 302/302 Vitest, 68/68 Playwright, 73/73 extension tests, 24/24 synthetic browser, build/typecheck/lint/Graph PASS. Existing-project Vercel production prebuild passed with zero tracked drift; exact source `8043886` then passed CI `34446809913`, deployed as `dpl_FGqGheG1GSx8EmUEJvd4rhEsdKYh`, and passed anonymous markers plus the complete rate-aware 68/68 public browser matrix. r10/r11 and the two monolithic public 67/68 rate-limit attempts remain retained historically. |
