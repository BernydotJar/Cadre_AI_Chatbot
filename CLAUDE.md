...
As of 2026-09-09, core N1–N6 remains complete and the separate productization graph is complete: P1/P2/P3 are DONE with every gate PASS. A separate owner-authorized premium-experience graph is active; PX2 is in REVIEW with `design-review=BLOCKED` because the attempted independent Granite critic produced no usable verdict. A protected Vercel review preview exists at `https://cadre-ai-chatbot-4xso24ztz-cadre-ai3.vercel.app` on the existing project; it is WIP and must not be promoted until the design gate passes. Current source verification is 279/279 Vitest and 52/52 local Playwright; the clean external Playwright matrix also passes 52/52 against the existing Vercel alias after promotion of source `7b6004c1fa5b715b1c2775d07981ebac1eee8622` to deployment `dpl_DMA3WxfAfMgwc7kZLueKctx6kfsy`. Donna/product markers, one-question behavior, explicit opt-out and pricing boundary were anonymously reverified. G9 and G11 are DONE at their declared scopes. G10 remains blocked only at remote CI/CD activation because the dedicated audited Git publication mechanism still lacks its platform-managed GitHub credential; shared/container GitHub authentication does not satisfy that publisher. Do not add product scope or create replacement infrastructure to work around it. Use `docs/release-runbook.md` and `progress/checkpoint.md` to resume Git publication/packaging work.
