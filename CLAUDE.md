...
- Source publication uses only the dedicated audited Git publication mechanism and remains fast-forward only. The publisher credential was restored on 2026-09-09. The last audited remote premium tip is `8ae8a3a1b0e53100218930aa893eed90a8592562`; GitHub CI run `34405396973` passed on that SHA. Production workflow run `34405608879` then stopped at the existing-project Vercel binding preflight because its three production binding values were absent; manual existing-project deployment and anonymous 58/58 public verification passed on the same runtime source. Do not bypass the repository release gate. The premium graph is now complete; automated production delivery still requires an explicit existing-project GitHub production-secret binding. Scope/schema changes, dependencies, extra spending and final closure still need human approval. External upload/submission, Web Store publication, paid add-ons/domains and real n8n delivery credentials remain separate unauthorized actions.
...
As of 2026-09-09, core N1–N6, productization P1/P2/P3, and premium PX2/PX3/PX3B/PX4/PX5 are complete. PX5 closed at **revision 4** after retaining and repairing a real post-deployment ambient Pause/autoplay cold-load failure; revision 4 also repairs `npm run release:gate` so post-DONE Graph invalidation revokes stale DONE/PASS state. Fresh detached verification passes 288/288 Vitest, 58/58 Playwright, 72/72 extension tests, 24/24 synthetic browser, build/typecheck/lint, Git integrity, and Graph validation. Repaired source `8ae8a3a1b0e53100218930aa893eed90a8592562` passed GitHub CI run `34405396973`, is deployed on the existing Vercel project as `dpl_EECF4m6NbwdN73j6Z2JxSfVpWbP7`, and passes anonymous product/API smoke plus the full 58/58 public Playwright matrix. G10 remains blocked only at automated deploy-check because GitHub `production` lacks the existing Vercel binding; do not create replacement infrastructure or bypass the repository release gate.

**PX6 cinematic/proactive contract (owner-authorized 2026-09-09).** The public page is website-first and the chat is a floating Donna product surface. `ClientConfig.publicHighlights` is the only source for surrounding business proof/product facts; `PersonaProfile.boundaryVoice` can add only short empathy tone, and `ExperienceProfile.quickPrompts` owns suggested questions. Pricing stays a deterministic decline with verified business-outcome framing and an official handoff, never an invented number. New motion is limited to Donna's idle/shaping signal orb plus one launcher glare sweep; existing local ambient video remains the only cinematic media layer. Reduced motion disables the new animation. Keep `docs/cinematic-proactive-experience.md` synchronized with the implementation. Product tests are intentionally run only after the assembled producer change is complete; then use a real independent critic (Claude Code when authenticated/available), fixer if needed, independent verifier, and Graph gates before release.
