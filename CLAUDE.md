...
## Current product contract — 2026-09-10
...
1. **Public web assistant — mandatory.** The active allowlisted product is `cadre-donna`: Cadre typed curated knowledge + deterministic safety/routing + the English **Donna** persona + a profile-driven web experience. Donna answers first and may add at most one configured diagnostic question for a grounded topic; explicit latest-turn requests such as `just answer`/`no follow-up` suppress that optional question. For pricing/unsupported/account/unknown boundaries, the deterministic policy still owns the decision, facts and handoff; `PersonaProfile.boundaryVoice` may add only a short tone-only empathy lead. The visible assistant label must match `persona.name`. PX6 uses the original profile-driven `signal-orb`, high-value `quickPrompts`, and a floating chat panel over a website-first shell; exact ordinary greetings remain whole-message deterministic welcomes and cannot bypass boundaries.
...
- **Knowledge changes** belong in `ClientConfig` and require source/provenance review. `officialDomain` remains the primary link authority; use `additionalOfficialDomains` only for explicitly observed client-linked hosts, keep the list small, and prefer the narrowest host actually observed rather than trusting a sibling domain wholesale.
...
- Source publication uses only the dedicated audited Git publication mechanism and remains fast-forward only. The publisher credential was restored on 2026-09-09. Final PX6 runtime source `80438863ee91e18fcc4fb070e360e9754b52e887` reached remote `main` through the audited path and GitHub CI run `34446809913` passed. Production workflow run `34447034685` then stopped safely at the existing-project Vercel binding preflight because `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, and `VERCEL_TOKEN` were absent. The same exact source was subsequently delivered manually through the already-authorized existing Vercel project as `dpl_FGqGheG1GSx8EmUEJvd4rhEsdKYh` and passed public equivalence. Later commits may contain release evidence/documentation only; they are not new runtime source unless application files change. Do not bypass the repository release gate; automated production delivery still requires an explicit existing-project GitHub production-secret binding. Scope/schema changes, dependencies, extra spending and final closure still need human approval. External upload/submission, Web Store publication, paid add-ons/domains and real n8n delivery credentials remain separate unauthorized actions.
...
As of 2026-09-10, core N1–N6, productization P1/P2/P3, and premium PX2/PX3/PX3B/PX4/PX5 remain complete. PX6 is **DONE + PUBLIC PASS at revision 12**. Exact runtime source `8043886` passed GitHub CI run `34446809913` and was manually delivered through the already-linked existing Vercel project as `dpl_FGqGheG1GSx8EmUEJvd4rhEsdKYh` after automated production workflow `34447034685` failed closed only on missing GitHub production Vercel binding values. Anonymous markers pass, and the complete current external browser coverage passes **68/68** when executed in two fresh client-rate windows (34 desktop + 34 mobile) because production intentionally allows 10 requests per 60 seconds per client IP. Two monolithic 67/68 runs are retained as correct 429/retry evidence; no product rate limit or test was weakened. Detached local verification passes 302/302 Vitest, 68/68 Playwright, extension build, 73/73 extension tests, 24/24 synthetic extension browser, build/typecheck/lint, and Graph validation. PX5 remains a historical rollback line, not the active production product. G10 remains blocked only for **automated** deployment because GitHub `production` lacks the existing Vercel binding; manual exact-project delivery and public runtime equivalence are proven. Do not create replacement infrastructure or bypass the repository release gate.

**PX6 cinematic/proactive contract (owner-authorized 2026-09-09; source refresh 2026-09-10).** The public page is website-first and the chat is a floating Donna product surface. `ClientConfig.publicHighlights` is the only source for surrounding business proof/product facts; approved client links may use only the primary official host or an explicitly configured delegated host, and the active Cadre config currently delegates the exact observed `portal.gocadre.ai` host without treating it as a verified private client-login portal; `PersonaProfile.boundaryVoice` can add only short empathy tone, and `ExperienceProfile.quickPrompts` owns suggested questions. Pricing stays a deterministic decline with verified business-outcome framing and an official handoff, never an invented number. New motion is limited to Donna's idle/shaping signal orb plus one launcher glare sweep; existing local ambient video remains the only cinematic media layer. Reduced motion disables the new animation. Keep `docs/cinematic-proactive-experience.md` synchronized with the implementation. Product tests are intentionally run only after the assembled producer change is complete; then use a real independent critic (Claude Code when authenticated/available), fixer if needed, independent verifier, and Graph gates before release.
