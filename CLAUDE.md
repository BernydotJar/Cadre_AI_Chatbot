...
As of 2026-09-10, core N1–N6, productization P1/P2/P3, and premium PX2/PX3/PX3B/PX4/PX5 remain complete. PX5 revision 4 (`8ae8a3a1b0e53100218930aa893eed90a8592562`, deployment `dpl_EECF4m6NbwdN73j6Z2JxSfVpWbP7`) remains the verified public rollback baseline with anonymous 58/58 Playwright PASS. PX6 revision 10 reached local `design-review`, `verification`, and `integration-proof` PASS with 301/301 Vitest + 68/68 Playwright, and closure source `69c590b` passed GitHub CI run `34442075458`; automated deployment run `34442247941` failed closed only because the GitHub `production` Vercel binding is absent. Before PX6 public equivalence, a fresh public-source audit changed the published results/Maturity destination, so the Graph was correctly reopened to **PX6 revision 11 / repair_required**. Revision 11 is limited to source/provenance/link-allowlist/documentation consistency; prior revision-10 PASS evidence is historical until fresh critic/verifier/gates close the new revision. G10 remains blocked only at automated deploy-check; do not create replacement infrastructure or bypass the repository release gate.
