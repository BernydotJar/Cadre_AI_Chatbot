...
**CURRENT GRAPH STATE: PX6 REVISION 8 / REPAIR_REQUIRED.** Revision 7 passed independent source review, then its detached clean verifier passed Git integrity, locked install, typecheck, lint, 301/301 Vitest, and production build before Playwright finished **61/68 PASS**. The seven retained browser failures are now the only PX6 repair scope: four 320/360px text-spacing cases where the floating sheet collapsed the conversation region to 0, one mobile 15px composer regression, and two mobile requests that hit the shared local E2E rate-limit bucket after desktop. No PX6 verification, integration-proof, deployment, or release PASS is claimed. The exact next action is the revision-8 accessibility/test-isolation repair, independent source review, then a fresh clean verifier.

The paragraph below is the retained revision-7 pre-verifier snapshot and is superseded by the current Graph state above; it remains until revision-8 verification closes so the failed stage is not rewritten out of the resumable history.
...
Premium graph sequence at this checkpoint is **177**, last event `e3c9f3bf-3c1a-4ddb-83a9-24feb4de099e`; `PX6-cinematic-proactive-donna` is revision **8 / repair_required**. The r4 and r7 failure evidence, r5 focused reproduction, and r6 critic remain append-only historical evidence rather than being rewritten as PASS.
