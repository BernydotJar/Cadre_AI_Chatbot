...
- Release safety repair adds `npm run release:gate` before dependency install/Vercel credentials. If `progress/premium-graph.*` exists, every declared premium node must be DONE and no latest gate may be FAIL/BLOCKED; PX5 is still incomplete, so the repository release gate remains fail-closed by design until PX5 reaches DONE.
