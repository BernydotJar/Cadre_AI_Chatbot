...
- Release safety repair adds `npm run release:gate` before dependency install/Vercel credentials. If `progress/premium-graph.*` exists, every declared premium node must be DONE and no latest gate may be FAIL/BLOCKED; all premium nodes are DONE with current gates PASS, so the repository release gate now passes; automated CD still requires the existing-project GitHub production secrets.
