...
**PREMIUM_EXPERIENCE_PX2_PX3_PX3B_PX4_PX5_REV4_DONE — RELEASE GATE PASS / REDEPLOY PENDING; GITHUB CI ACTIVE.** Core N1–N6 and productization P1/P2/P3 remain complete. Premium revision 3 `ded08392e2dfea76e4050a234165f5e7b8c4152b` passed GitHub CI run `34403021494` and was deployed to the existing Vercel project as `dpl_A6CvaDHT34auCMFFoNxnmDvCXCBY`. Anonymous page/API smoke passed, but the first full public browser matrix returned **57/58** because a visible Pause action could observe native `video.paused=true` while autoplay was still pending and issue Play instead. PX5 was invalidated to revision 4. The repair makes application state authoritative for the control, adds a deterministic cold-load regression, and fixes the repository release gate so post-DONE `failure.recorded`/`node.invalidated` events revoke stale release state. Fresh detached verification passes **288/288 Vitest, 58/58 Playwright, 72/72 extension tests, 24/24 synthetic browser**, build/typecheck/lint, Git integrity, and Graph validation. PX5 revision 4 is DONE with all gates PASS and `npm run release:gate` passes; fixed public equivalence is pending audited publication/CI/redeploy. Automated GitHub CD remains blocked separately at the existing-project binding preflight because GitHub `production` lacks `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, and `VERCEL_TOKEN`. See `progress/checkpoint-premium.md` for the premium failure/repair chain.
...
- `.github/workflows/ci.yml`: now active remotely. Premium revision-3 push run `34403021494` on source `ded08392e2dfea76e4050a234165f5e7b8c4152b` completed SUCCESS, including quality and browser jobs.
- `.github/workflows/deploy-production.yml`: exact-SHA production delivery after successful main CI or manual dispatch. Premium revision-3 production run `34403242807` stopped at `Require an existing Vercel project binding` because all three GitHub Actions values were empty; no Vercel pull/build/deploy step ran.
...
- Existing manual Vercel project binding and the productized public deployment remain proven separately. The premium graph is releasable again at revision 4, but adding GitHub production secrets remains a separate infrastructure action rather than a prerequisite for manual existing-project delivery.
...
The audited Git publisher is restored and remains fast-forward only; no shell-token workaround or history reconstruction was used. Premium revision-3 source `ded08392e2dfea76e4050a234165f5e7b8c4152b` reached remote `main` through that channel, and GitHub CI run `34403021494` completed successfully. The automatically triggered production workflow run `34403242807` failed safely at its existing-project binding preflight because `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, and `VERCEL_TOKEN` were absent from the GitHub `production` environment. This remains the external G10 automated-deployment blocker; premium Graph incompleteness is no longer a blocker.

## Exact resume path

1. Publish the final premium closure SHA through the audited Git path and require remote CI PASS.
2. Deploy the exact reviewed SHA through the already-bound Vercel project and verify anonymous public markers + the 58-case browser matrix. GitHub automated CD remains separately blocked until the existing Vercel org/project/token binding is supplied as `production` secrets.
