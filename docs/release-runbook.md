...
| GitHub remote activation | **ACTIVE; latest pre-r11 closure CI PASS** | source `69c590b` published through the audited channel; CI run `34442075458` passed. PX6 was then reopened for revision-11 source freshness, so `69c590b` is historical rather than the final candidate. |
...
| Premium PX6 | **revision 11 source-freshness repair in progress; revision 10 evidence retained as historical** | revision 10 previously reached all three premium gates PASS locally (clean verifier **301/301 Vitest + 68/68 Playwright**), and closure source `69c590b` passed GitHub CI run `34442075458`; deploy-production run `34442247941` then failed closed on the same missing Vercel org/project/token binding as G10. On 2026-09-10 the Graph was reopened to `repair_required` because the `portal`/`maturity-index`/`track-ai-results` facts and links were stale against the live public site (the "Get Your AI Results" CTA now resolves to a delegated `portal.gocadre.ai` domain). This repair updates `src/config/cadre.ts`/`types.ts` and regression tests; it does not itself claim a new PASS gate, a new deploy, or public equivalence — those remain pending after critic/verifier/Graph gate closure. |
